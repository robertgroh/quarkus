...
[TIP]
====
You can also run a Quarkus application in debug mode with a suspended JVM using `./mvnw compile quarkus:dev -Ddebug -Dsuspend` which is a shorthand for `./mvnw compile quarkus:dev -Ddebug=true -Dsuspend=true`.
